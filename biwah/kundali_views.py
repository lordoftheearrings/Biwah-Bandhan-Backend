...
import os
from django.conf import settings
from .models import User, Kundali
...
class GenerateKundaliView(APIView):
    def post(self, request, *args, **kwargs):
        ...
        # Get or create the Kundali instance for the user
        kundali, created = Kundali.objects.get_or_create(user=user)
        ...
        # Update Kundali data and save
        kundali.birth_year = year
        kundali.birth_month = month
        kundali.birth_date = day
        kundali.birth_hour = hour
        kundali.birth_minute = minute
        kundali.birth_second = second
        kundali.birth_location = birth_location
        kundali.birth_latitude = latitude
        kundali.birth_longitude = longitude
        kundali.kundali_svg = kundali_svg_file_path
        kundali.save()
...
class RetrieveKundali(APIView):
    def post(self, request, *args, **kwargs):
        ...
        # Get the Kundali instance for the user
        kundali = Kundali.objects.filter(user=user).first()

        if not kundali:
            return Response({"message": "Kundali not found"}, status=status.HTTP_404_NOT_FOUND)

        # Retrieve the kundali SVG file field
        kundali_svg = kundali.kundali_svg
